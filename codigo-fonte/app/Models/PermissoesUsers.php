...
/**
 * Class PermissoesUsers
 * @package App\Models
 * @author Thiago Farias <thiago.farias@jointecnologia.com.br>
 * @version 31/05/2017
 */
class PermissoesUsers extends ModelControl {
    
    public $table = 'permissoes_users';
    ...
    */
    ...
    public function atribuirPermissao($dados) {
        $permissoes = $this->where('user_id', $dados['user_id'])
    ...
    public static function setar() {
        $permissoes = self::join('permissoes', 'permissoes.id', '=', 'permissoes_users.permissao_id')
            ->select('permissoes.permissao')
            ->where('permissoes_users.user_id', \Auth::user()->id)
            ->pluck('permissao');
